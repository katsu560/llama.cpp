...
NOTEST="test-double-float test-opt test-c.o"
TARGETS=
TESTS=
...
get_targets()
{
	...
	BEGIN { ST=0; split(NOTGT0,NOTGT); NTGT=0 }
	function is_notgt(tgt) {
       		for(i in NOTGT) { if (NOTGT[i]==tgt) return 1; continue }
       		return 0;
	}
	function is_tgt(tgt) {
		for (i in TGT) { if (TGT[i]==tgt) return 1; continue }
		retuen 0;	
	}
ST==3 && /^tests\// { T=$0; T1=$1; sub(/^tests\//,"",T1); sub(/:$/,"",T1); if (is_notgt(T1)==0 && is_tgt(T1)==0) { printf("%s ",T1); NTGT=NTGT+1; TGT[NTGT]=T1 } }
	ST==2 && /^tests:/ { ST=3 }
	ST==1 && /^$/ { ST=2 }
	ST==1 && !/^$/ { T=$0; sub(/[\r\n]$/,"",T); sub(/^[ ]*/,"",T); sub(/\\\/,"",T); gsub(/tests\//,"",T); split(T,TGT0);
	  for(I in TGT0) { if (is_notgt(TGT0[I])==0) { printf("%s ",TGT0[I]); NTGT=NTGT+1; TGT[NTGT]=TGT0[I] } } }
...
do_main()
{
	...
	if [ ! x"$DOMAINOPT" = xNOEXEC ]; then
		if [ -f ./$DIRNAME/main -a ! x"$SUBOPT" = xGGUF ]; then
			#msg "./$DIRNAME/main -m ../models/ggml-alpaca-7b-q4.bin -n 512 $MAINOPT -s $SEED -p \"$PROMPT\""
			chk_level $LEVELMIN do_bin main ../models/alpaca/ggml-alpaca-7b-q4.bin PROMPT "-n 512 $MAINOPT"

			#
			chk_level $LEVELMIN do_bin main ../models/vicuna/ggml-vic7b-q4_0.bin PROMPT "-n 512 $MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/ggml-vic7b-q4_1.bin PROMPT "-n 512 $MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/vicuna/ggml-vic7b-q4_0-new.bin PROMPT "-n 512 $MAINOPT"
			#
			chk_level $LEVELMIN do_bin main ../models/wizardlm/WizardLM-7B-uncensored.ggml.q4_0.bin PROMPT "-n 512 $MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/wizardlm/WizardLM-7B-uncensored.ggml.q5_0.bin PROMPT "-n 512 $MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/wizardlm/WizardLM-7B-uncensored.ggml.q5_1.bin PROMPT "-n 512 $MAINOPT"

			chk_level $LEVELMIN do_bin main ../models/wizardlm/Wizard-Vicuna-13B-Uncensored.ggml.q4_0.bin PROMPT "-n 512 $MAINOPT"

			# vicuna 1.1 2023.6
			#msg "./$DIRNAME/main -m ../models/vicuna-7b-1.1.ggmlv3.q2_K.bin $MAINOPT $OPT -s $SEED -p \"$PROMPT\""
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q2_K.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q3_K_L.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELSTD do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q3_K_M.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q3_K_S.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q4_0.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q4_1.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMIN do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q4_K_M.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q4_K_S.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q5_0.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q5_1.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELSTD do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q5_K_M.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q5_K_S.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMAX do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q6_K.bin PROMPT "$MAINOPT $OPT"
			chk_level $LEVELMIN do_bin main ../models/vicuna/vicuna-7b-1.1.ggmlv3.q8_0.bin PROMPT "$MAINOPT $OPT"

			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-small-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-small-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-small-q4_0.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-medium-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-medium-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-medium-q4_0.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-large-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-large-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-large-q4_0.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-1b-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-1b-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-1b-q4_0.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-3b-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-3b-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-3b-q4_0.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMAX do_bin main ../models/cyberagent/ggml-calm-7b-f32.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELSTD do_bin main ../models/cyberagent/ggml-calm-7b-f16.bin PROMPTJP "$MAINOPT"
			chk_level $LEVELMIN do_bin main ../models/cyberagent/ggml-calm-7b-q4_0.bin PROMPTJP "$MAINOPT"
...
usage()
{
	echo "usage: $MYNAME [-h][-v][-n][-f][-nd][-ncp][-nc][-ts][-noavx|avx|avx2][-ione][-sync][-sycl][-qkk64][-lv LEVEL][-s SEED] dirname branch cmd"
	echo "       $MYNAME setup"
	...
	echo ""
	echo "  cmd ... setup .. setup $TOPDIR, git clone, init, download scripts"
